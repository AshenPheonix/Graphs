...
class Node():
    ...
    def __init__(self, fromKey,fromDir):
        self.back=(fromDir,fromKey)
        self.connected={
            'n':None,
            'e':None,
            'w':None,
        ...
        if fromKey is not None:
            self.connect(fromKey,fromDir)
    
    def connect(self, connectedKey,connectedDir):
        self.connected[connectedDir]=connectedKey

    def __str__(self):
        return f"Node back data is {self.back} and connected is n: {self.connected.get('n')}, s: {self.connected.get('s')}, e: {self.connected.get('e')}, w: {self.connected.get('w')}"
    def __repr__(self):
        return f"Node back data is {self.back} and connected is n: {self.connected.get('n')}, s: {self.connected.get('s')}, e: {self.connected.get('e')}, w: {self.connected.get('w')}"
...
while len(toVisit)>0:
    current=toVisit.pop()
    exits=roomGraph[current][1]
    node=paths[current]
    switch=False

    if exits.get('n') and not exits.get('n') in paths:
        temp=Node(current,'s')
        node.connect(exits.get('n'),'n')
        paths[exits.get('n')]=temp
        toVisit.append(exits.get('n'))
        switch=True

    if exits.get('e') and not exits.get('e') in paths:
        temp=Node(current,'w')
        node.connect(exits.get('e'),'e')
        paths[exits.get('e')]=temp
        toVisit.append(exits.get('e'))
        switch=True

    if exits.get('w') and not exits.get('w') in paths:
        temp=Node(current,'e')
        node.connect(exits.get('w'),'w')
        paths[exits.get('w')]=temp
        toVisit.append(exits.get('w'))
        switch=True

    if exits.get('s') and not exits.get('s') in paths:
        temp=Node(current,'n')
        node.connect(exits.get('s'),'s')
        paths[exits.get('s')]=temp
        toVisit.append(exits.get('s'))
        switch=True

    if not switch and len(toVisit)>0:
        found=False
        while not found:
            traversalPath.append(node.back[0])
            node=paths[node.back[1]]
            if toVisit[-1] in node.connected.values():
                found=True
    if len(toVisit)>0:
        fromHere=paths.get(toVisit[-1]).back[0]
        if fromHere=='n':
