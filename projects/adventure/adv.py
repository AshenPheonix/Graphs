...
class Node():
    ...
    def question(self,fromDir,backList):
        if self.visited:
            return False
        else:
            backList.append(fromDir)
            return True

    def hasNeighbor(self,target):
        if target in self.connected.values():
            return True
        else:
            return False

    def whichNeighbor(self,target):
        if not self.hasNeighbor(target):
            return None
    ...
    def depth(self,rooms,inc,me):
        ret = []
        toVisit=[me]
        while len(toVisit)>0:
            current=toVisit.pop()
            if current != inc and not current in ret:
                for room in rooms[current][1].values():
                    if not room in ret:
                        ret.append(room)
                        toVisit.append(room)
        return len(ret)
...
def BFS(key1,key2,roomList):
    ...
    while toVisit.size>0:
        ...
        if current.data==key2:
            ...
            for key,value in node.getNeighbors():
...
while len(toVisit)>0:
    ...
    for index,direction in enumerate(directions):
        ...
        if target and not target in paths:
            temp=Node(current,oppDir[index])
            node.connect(target,direction)
            paths[target]=temp
            toVisit.append(target)
            switch=True;
            times+=1
        elif target and target in toVisit:
            toVisit.remove(target)
            toVisit.append(target)
            node.connect(target,direction)
            paths[target].connect(current,oppDir[index])
            switch=True
            loopDir=direction
            loop=True
    ...
    if not switch and len(toVisit)>0:
        ...
        while not found:
            ...
            if toVisit[-1] in node.connected.values():
                found=True
    if len(toVisit)>0 and not loopDir:
        direction = node.whichNeighbor(toVisit[-1])
        traversalPath.append(direction)
    elif loopDir != '?' and loopDir != None:
